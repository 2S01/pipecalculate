#pyinstaller -w XXX.py --hidden-import lxml._elementpath
...
class ActivationWidget(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.activationcode.setObjectName('activationcode')
        lb2 = QLabel('请输入激活码:')
    ...
    def verify(self):
        licence = self.get_value('activationcode')
        with open('licence.txt', "w",encoding='UTF-8') as file:
            file.write(licence)
        if self.get_verify_code() == licence:
            bindwidget.hide()
            demo.show()
        else:
...
class DemoMain(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.lineEdit_4.editingFinished.connect(self.show_node)
        self.lineEdit_3.editingFinished.connect(self.show_node2)
        ...
        if self.data_path:
            ...
            try:
                ...
                if directory:
                    ...
                    file_dict = {os.path.join(directory, i+'.prd'): os.path.join(directory, j+'.fre') for i in p0 for j in f0 if i == j}
                    if file_dict:
                        ...
                        for prd_path, fre_path in file_dict.items():
                            ...
                            if len(total_mass.keys()) == len(frequency.keys()):
                                ...
                                shock_path, shock_lines = ShockSpectrum().cal_a_v(total_mass, frequency, new_shock_dir, shotname)
    ...
    def show_node_end(self, widget1, widget2, widget3):
        try:
            ...
            print(old_node)
            all_pt, all_pt1, hanger_line = self.get_pt_fre()
            print(all_pt)
            print(all_pt1)
            # pt = 124
            options_node = all_pt - all_pt1
            old_node = all_pt1 & set(old_node)
            print(old_node)
            widget1.setText(','.join(old_node))
            message1 = ''
            result_line = []
            for i in old_node:
                options_node.add(i)

                if str(i) not in all_pt1:
                    message1 += str(i) + ','
                ...
                for j, line in enumerate(hanger_line):
                    ...
                    if line.split()[0] and not line.startswith('*'):
                        ...
                        if m0 and (m1 or m2 or m3 or m4):
                            flag_line += '*' + line
        ...
        if directory:
            ...
            try:
                ...
                if max_filename_list:
                    self.data_dict, self.fre_less1, self.data_less1 = GetMaxResult(directory, max_filename_list, self.max_path).get_max_files()
        ...
        try:
            ...
            if self.data_path:
                ...
                if new_node and old_node and flag1 and flag2:
                    all_node_comb = self.make_comb(old_node, new_node)
                    for i in old_node:
                        flag = 'PT=' + str(i)
                    ...
                    if not self.RSTN.isChecked() and not self.SNUB.isChecked() and not self.RSUP.isChecked()\
                        ...
                        file_number = self.write_into_fre(file_number, new_fre_dir, self.max_file_list, self.fre_file_list, all_lines,
                                                          insert_index, lines)
            ...
            if self.data_path:
                ...
                if comb1 and comb2 and not comb3:
                    ...
                    if all_lines1 and all_lines2 and all_lines3:
                        total_lines = self.write_into_fre_file3(total_nodes, all_lines1, all_lines2, all_lines3, flag_line)
                        self.write_into_fre2(file_number, new_fre_dir, total_lines, insert_index, old_lines)
                        QMessageBox.information(self, '提示', '输入卡生成完毕！\n位置%s' % new_fre_dir)
    ...
    def get_pt_fre(self):
        ...
        if self.data_path:
            ...
            flag = "PT="
            for line in lines:
                m = re.search(flag, line, re.IGNORECASE)
                m1 = re.search('RSTN', line, re.IGNORECASE)
                m2 = re.search('SNUB', line, re.IGNORECASE)
                m3 = re.search('RSUP', line, re.IGNORECASE)
                m4 = re.search('HANG', line, re.IGNORECASE)
                m5 = re.search('ANCH', line, re.IGNORECASE)
                m6 = re.search('NOZZ', line, re.IGNORECASE)
                m7 = re.search('COOR', line, re.IGNORECASE)
                m8 = re.search('CONN', line, re.IGNORECASE)
                if not line.startswith('*') and m and not m1 and not m2 and not m3 and not m4 and not m5 and not m6 \
                        and not m7 and not m8:
                    params = line.split()
                    for p in params:
                        if re.search(flag, p, re.IGNORECASE):
                            pt_num = p.split('=')[1].strip()
                            all_pt.add(pt_num)
                if m and (m1 or m2 or m3 or m4) and not line.startswith('*'):
                    flag_line.append(line)
                    params = line.split()
                    for p in params:
                        if re.search(flag, p, re.IGNORECASE):
                            pt_num = p.split('=')[1].strip()
                ...
                if re.search('PB=', line, re.IGNORECASE):
                    params = line.split()
            ...
            return all_pt, all_pt1, flag_line
        else:
            QMessageBox.information(self, '提示', '请先配置fre文件')

    def write_into_fre(self, file_number, new_fre_dir, max_file_list, fre_file_list, data_lines, write_index, old_lines):
    ...
    def get_hanger_all(*args):
        total_list = []
        for w in itertools.product(*args):
            new_w = list(w)
            for x in w:
                if x[1] is None:
                    new_w.remove(x)
    ...
    def button_data_click(self):
        name, _ = QFileDialog.getOpenFileName(self, '选择fre数据文件', r'./', '数据文件(*.fre *.fre)')
    ...
    def button_save_conf(self):
        ...
        try:
            if os.path.exists(self.lineEdit_6.text()):
                ...
                self.dir_num = 1
                if cal_flag:
                    self.max_path, self.prr_path, self.ppo_path, self.prd_path = cal_flag[0], cal_flag[1], cal_flag[2], cal_flag[4]
                    ...
                    file_list = []
                    extensions = ['.fre', '.ppo', '.prc', '.prd', '.max', '.pre', '.pri', '.prl', '.prr', '.tmp', '.txt']
                    ...
                    try:
                        for root, dirs, files in os.walk(file_dir):
                            for file in files:
                                ...
                                if (fre_name == file_name and suf in extensions) or (file_name_shock == file_name and suf in extensions):
    ...
    def button_material_click(self):
        """打开材料库操作"""
        name, _ = QFileDialog.getOpenFileName(self, '打开材料库文件', r'./',
                                              '数据文件(*.xml)')
...
if __name__ == '__main__':
    ...
    with open('licence.txt', "r",encoding='UTF-8') as file:
        text = file.read()
    licence = bindwidget.get_verify_code()
    if text == licence:
        demo.show()
    else:
        bindwidget.show()
